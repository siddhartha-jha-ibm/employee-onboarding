...
public class AccessRevokedConsumer : BackgroundService
{
    private readonly IOnboardingSagaService _sagaService;
    private IConnection? _connection;
    private IChannel? _channel;

    public AccessRevokedConsumer(IOnboardingSagaService sagaService)
    {
    ...
    protected override async Task ExecuteAsync(CancellationToken stoppingToken)
    {
        ...
        {
            var @event =
                JsonSerializer.Deserialize<AccessRevokedIntegrationEvent>(
                    ea.Body.Span);

            if (@event != null)
            {
                await _sagaService.HandleAccessRevokedAsync(@event);
