...
public class AccessProvisionedConsumer : BackgroundService
{
    private readonly IOnboardingSagaService _sagaService;
    private IConnection? _connection;
    private IChannel? _channel;

    public AccessProvisionedConsumer(IOnboardingSagaService sagaService)
    {
        ...
        consumer.ReceivedAsync += async (_, ea) =>
        {
            var @event =
                JsonSerializer.Deserialize<
                    AccessProvisionedIntegrationEvent>(
                        ea.Body.Span);

            if (@event != null)
            {
                await _sagaService
                    .HandleAccessProvisionedAsync(@event);
